...
export default function SignIn() {
  ...
  const [email,setEmail] = useState(""), [password,setPassword] = useState(""), [mode,setMode] = useState<"in"|"up">("in")
  const [err,setErr] = useState<string|null>(null), [busy,setBusy] = useState(false)
  const router = useRouter()

  useEffect(()=>{ if(!loading && user) router.replace("/") },[user,loading,router])

  async function onSubmit(e:FormEvent){ e.preventDefault(); setBusy(true); setErr(null)
    try{ mode==="in" ? await signInEmail(email,password) : await signUpEmail(email,password); router.replace("/") }
        ...
        <div className="w-full max-w-sm rounded-2xl border border-white/20 bg-white/10 dark:bg-black/30 backdrop-blur-xl shadow-[0_20px_60px_-20px_rgba(0,0,0,.45)]" suppressHydrationWarning>
          <div className="p-6 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold tracking-tight">{mode==="in"?"Sign In":"Create Account"}</h1>
                <p className="text-sm text-black/60 dark:text-white/60">{mode==="in"?"Welcome back — let’s get you in.":"Join and start tracking smarter."}</p>
              </div>
            </div>
            ...
            <form onSubmit={onSubmit} className="space-y-3" suppressHydrationWarning>
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-black/70 dark:text-white/70">Email</label>
                <input className="border border-white/30 bg-white/70 dark:bg-white/10 backdrop-blur rounded-xl w-full px-3 h-11 outline-none focus:ring-2 focus:ring-indigo-400/60 transition" placeholder="your_name@example.com" value={email} onChange={e=>setEmail(e.target.value)} suppressHydrationWarning />
              </div>
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-black/70 dark:text-white/70">Password</label>
                <input className="border border-white/30 bg-white/70 dark:bg-white/10 backdrop-blur rounded-xl w-full px-3 h-11 outline-none focus:ring-2 focus:ring-emerald-400/60 transition" placeholder="••••••••" type="password" value={password} onChange={e=>setPassword(e.target.value)} suppressHydrationWarning />
              </div>
              {err && <p className="text-red-600 text-sm">{err}</p>}
              <button disabled={busy} className="cursor-pointer h-11 w-full rounded-xl bg-black text-white dark:bg-white dark:text-black font-medium shadow-[0_12px_30px_-12px_rgba(0,0,0,.6)] hover:opacity-95 active:scale-[.99] transition" suppressHydrationWarning>{busy?"Please wait…":(mode==="in"?"Sign In":"Sign Up")}</button>
            </form>

            <div className="flex items-center justify-between">
              <button className="cursor-pointer text-sm underline underline-offset-4 hover:opacity-80" onClick={()=>setMode(mode==="in"?"up":"in")} suppressHydrationWarning>{mode==="in"?"Create account":"I already have an account"}</button>
              <a href="/" className="cursor-pointer text-sm underline underline-offset-4 hover:opacity-80" suppressHydrationWarning>Back</a>
            </div>
          </div>
        </div>
      </div>
